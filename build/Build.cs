...
sealed class Build : NukeBuild
{
    ...
    // Weaver emits this header from the qyl registry's `typespec` template: one line naming the
    // Weaver binary, the resolved registry's schema URL, the core semantic-conventions tag it
    // resolved against, and the GenAI registry commit. The `Schema:` line must repeat the core
    // tag; the backreference is what makes a hand-edited or half-regenerated header fail here.
    static readonly Regex OtelKeysProvenanceHeaderPattern = new(
        @"\A// <auto-generated/>\n" +
        $@"// Generated by Weaver (?<weaverVersion>{SemVerPattern}) from qyl registry " +
        $@"https://qyl\.at/schemas/(?<qylSchemaVersion>{SemVerPattern}) " +
        $@"\(core v(?<coreVersion>{SemVerPattern}), genai (?<genAiCommit>[0-9a-f]{{40}})\)\n" +
        @"// Schema: https://opentelemetry\.io/schemas/\k<coreVersion>\n" +
        @"// GenAI schema: https://opentelemetry\.io/schemas/[^\r\n]+\n" +
    ...
    // The semantic-conventions repository owns the exact registry pins and generated payload;
    // Weaver is the only generator there and the committed keys file is its `typespec` template
    // output. Keep this consumer-side gate version-agnostic: it validates complete, internally
    // consistent provenance. Migration and release orchestration separately byte-compares the
    // committed file with that producer's committed generated/typespec/otel-keys.gen.tsp.
    Target VerifyKeysProvenance => _ => _
        .Description("Validate generated/otel-keys.gen.tsp has a complete producer provenance header.")
        ...
        .Executes(() =>
        {
            ...
            if (!File.Exists(committed))
            {
                throw new InvalidOperationException(
                    $"VerifyKeysProvenance: committed keys file '{committed}' is missing.");
            }

            var header = File.ReadAllText(committed).ReplaceLineEndings("\n");
            var provenance = OtelKeysProvenanceHeaderPattern.Match(header);
            if (!provenance.Success)
            {
                throw new InvalidOperationException(
                    $"VerifyKeysProvenance: '{committed}' does not have the expected Weaver header with " +
                    "a Weaver version, a qyl registry schema URL, a core SemVer tag matching the schema " +
                    "line, and a full GenAI registry commit. Regenerate it in the semantic-conventions " +
                    "repository (scripts/generate.sh) and copy that repository's committed " +
                    "generated/typespec/otel-keys.gen.tsp here.");
            }

            Log.Information(
                "VerifyKeysProvenance: '{Committed}' was emitted by Weaver {WeaverVersion} from qyl registry {QylSchemaVersion} (core semconv v{Version}, GenAI registry {GenAiCommit}).",
                committed,
                provenance.Groups["weaverVersion"].Value,
                provenance.Groups["qylSchemaVersion"].Value,
                provenance.Groups["coreVersion"].Value,
                provenance.Groups["genAiCommit"].Value);
        });
    ...
    Target CompileDomainSpec => _ => _
        ...
        .DependsOn(VerifyKeysProvenance)
        .Executes(() =>
        {
            var spec = DomainSpecRoot;
            NpmRun(s => s
