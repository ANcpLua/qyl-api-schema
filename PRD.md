...
**Status:** Implemented, validated, and synchronized to `main`
**Revision:** 2026-08-08; journal authority, DuckDB.NET 1.5.5 runtime appender evidence, transactional rollback, generated binders, and persisted-index verification were refreshed and synchronized.
**Target repositories:**
...
| `qyl` journal-authority follow-up | `3c617a26a9b4693de0a2d70b82902cb8a352245a` | `main` synchronization only | Direct-main follow-up that makes run summaries journal-derived, adds runtime BLOB/rollback evidence, generated transactional binders, and exact persisted-index verification. No package release or deployment was required for this source rewrite. |
...
| 1. Product outcome | `satisfied` | `qyl-api-schema/api/workflow.tsp` and `models/workflow.tsp` own the public contract. `workflow_runs` now retains the immutable run envelope, coordination fields, generation/deletion fencing, and checkpoint metadata; disposable `workflow_run_summaries` is rebuilt from ordered `workflow_events` and supplies lifecycle state, journal head, counters, and projection input. |
...
| 8. DuckDB.NET 1.5.5 | `satisfied` | `Version.props` pins 1.5.5. Generated `AppendRow<TState>` uses reusable rows and static callbacks on the production journal path. A compiled generated assembly appends `byte[]` values through the native BLOB path against real DuckDB and verifies exact round trips. Conflict-sensitive encrypted content retains parameterized `ON CONFLICT` SQL. Forced mid-batch appender disposal proves rollback of journal rows, head, acknowledgement, manifest state, and preceding encrypted-content writes; the disposed appender is not reused. |
| 9. Schema generation | `satisfied` | Canonical table metadata now emits quoted column constants, named transactional column sets, and typed positional binders used by journal-head, manifest CAS, retention, and coordination operations. Startup compares the complete explicit index set by name, owning table, uniqueness, and ordered columns; authoritative mismatches with data fail closed and preserve the journal. |
...
| 13. Required tests | `satisfied` | The refreshed `Qyl.Collector.Tests` suite passed 207/207. Focused real-DuckDB coverage proves disposable-summary reconstruction, runtime BLOB appends, mid-batch transactional rollback/disposal, binder rename/reorder tracking, and fail-closed handling for missing, extra, wrong-owner, wrong-column, wrong-order, and non-unique journal indexes. Historical NativeAOT, browser, and package-consumer evidence below remains unchanged. |
...
| One persisted workflow authority | The append-only journal is the sole semantic history; run summaries, graph, and checkpoint state are reconstructable derivatives. Startup discards and rebuilds the run-summary projection from the immutable envelope plus ordered journal events. |
...
| DuckDB.NET 1.5.5 API adoption | Production journal appenders, executable native BLOB generation, rollback/disposal, Arrow streaming, typed failure classification, and the retained parameterized conflict paths are validated. |
| Unified generated storage surface | Canonical DDL, hashes, appenders, Arrow readers, mappings, transactional column sets/binders, and exact explicit-index verification derive from one metadata model. |
...
| Behavioral and platform validation | Refreshed on 2026-08-08: the qyl Release solution build completed with 0 errors; the current collector suite passed 207/207; focused journal-authority, generator, BLOB/rollback, and index coverage passed 67/67; and `./build.sh Check` passed all 15 schema targets. Historical NativeAOT, browser, cross-RID, and published-package evidence remains the prior release evidence described below. |
...
| Clean synchronized repositories | The scoped qyl implementation is committed as `3c617a26a9b4693de0a2d70b82902cb8a352245a` and pushed to `origin/main`; the PRD follow-up is committed and pushed separately in `qyl-api-schema`. Pre-existing local `.codex` edits in qyl were excluded from the implementation commit. |
