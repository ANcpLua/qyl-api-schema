...
sealed class Build : NukeBuild
{
    ...
    static readonly Regex OtelKeysProvenanceHeaderPattern = new(
        ...
        @"//   open-telemetry/semantic-conventions v(?<coreVersion>(?:0|[1-9][0-9]*)\.(?:0|[1-9][0-9]*)\.(?:0|[1-9][0-9]*)(?:-[0-9A-Za-z-]+(?:\.[0-9A-Za-z-]+)*)?) \((?<coreCommit>[0-9a-f]{40})\)\n" +
        @"//   open-telemetry/semantic-conventions-genai (?<genAiCommit>[0-9a-f]{40}) \(\k<genAiCommit>\)\n" +
    ...
    // Keep this consumer-side gate version-agnostic: it validates complete, internally
    // consistent provenance. Migration and release orchestration separately byte-compares the
    // committed file with emit_typespec_keys.py --stdout from that producer.
    Target VerifyKeysProvenance => _ => _
        ...
        .Executes(() =>
        {
            ...
            {
                throw new InvalidOperationException(
                    $"VerifyKeysProvenance: '{committed}' does not have the expected producer header with " +
                    "a full core SemVer tag and commit, a full GenAI commit, and matching schema version. " +
                    "Regenerate it with the semantic-conventions producer and byte-compare that producer's " +
                    "emit_typespec_keys.py --stdout output before release.");
            }

            Log.Information(
                "VerifyKeysProvenance: '{Committed}' declares core semconv v{Version} at {CoreCommit} and GenAI registry {GenAiCommit}.",
                committed,
                provenance.Groups["coreVersion"].Value,
                provenance.Groups["coreCommit"].Value,
