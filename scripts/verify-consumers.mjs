  ...
  try {
    ...
    await writeFile(
      ...
      `import { GenAiEtlCandidateStatusValues, GenAiEtlEvidenceSignalValues, GenAiEtlFrontierCostBasisValues, GenAiEtlValidationMetricValues, HealthStatusValues, ModelCatalogBillingModeValues, ModelCatalogMatchKindValues, ModelCatalogObservedIdentityBasisValues, ModelCatalogPriceSemanticsValues, ModelCatalogRateRelationValues, ModelCatalogTokenExclusionReasonValues, ProblemDetailsMediaType, ProviderBillingAttributionValues, ProviderBillingSourceStatusValues, RunnerMcpEvaluationExportFormatValues, RunnerMcpExecutionStatusValues, RunnerMcpTransportKindValues, RunnerResourceKindValues, RunnerResourceLifecycleValues } from ${JSON.stringify(`${npmPackage}/types`)};\n` +
        ...
        `if (ProblemDetailsMediaType !== "application/problem+json" || !errorsOwnProblemJson || !workbenchCookieAuth || !typedBootstrapCookie || !logStreamCapacityResponseDeclared || !typedQueryValidationResponsesDeclared || !metricsListDeclared || !exactMetricContract || !eventLogContract || !operationBodyContracts || !losslessAttributeValue || !exactEntityRef || !exactServerConfigurationUnion || !exactAssertionUnion || !exactExportUnion || !opaqueSdkBoundaries || !compoundValidationMetrics || !qualifiedCandidate || !aggregateBilling || !requiredCatalogSources || !separatedCatalogEstimate || !catalogSummaryMetrics || !separatedFrontierBasis || GenAiEtlValidationMetricValues.calibrationError !== "calibration_error" || GenAiEtlValidationMetricValues.spanPrecision !== "span_precision" || GenAiEtlValidationMetricValues.spanRecall !== "span_recall" || HealthStatusValues.healthy !== "healthy" || RunnerResourceLifecycleValues.ready !== "ready" || RunnerResourceKindValues.stdio !== "stdio" || RunnerMcpTransportKindValues.streamableHttp !== "streamable_http" || RunnerMcpExecutionStatusValues.timedOut !== "timed_out" || RunnerMcpEvaluationExportFormatValues.report !== "report" || !schema.$defs["Mcp.Tools.FetchTelemetryInput"]) process.exit(1);\n`,
    );
    run("node", ["smoke.mjs"], npmDir);
    await writeFile(
      join(npmDir, "smoke.ts"),
      `import type { Attribute, AttributeValue, EntityRef, GaugeMetricPoint, HistogramMetricPoint, LogRecord, MetricDoubleValue, MetricIntegerValue, MetricPoint, Resource, SumMetricPoint } from ${JSON.stringify(`${npmPackage}/types`)};\n` +
        ...
        `void [eventLog, emptyAttribute, intAttribute, doubleAttribute, kvlistAttribute, entityRef, metricResource, integerValue, namedDouble, finiteGauge, noRecordedGauge, noRecordedSum, point, invalidAttribute, invalidNamedDouble, invalidIntegerValue, invalidHistogram, invalidTimestamp];\n`,
    );
    run(
      process.execPath,
      [
    ...
    await writeFile(
      ...
      `using System.Text.Json;\nusing System.Text.Json.Nodes;\nusing System.Text.Json.Serialization;\nusing ModelContextProtocol;\nusing ModelContextProtocol.Protocol;\nusing Qyl.Api.Contracts.Common.Errors;\nusing Qyl.Api.Contracts.Common.Pagination;\nusing Qyl.Api.Contracts.Cost;\nusing Qyl.Api.Contracts.Health;\nusing Qyl.Api.Contracts.Mcp.Tools;\nusing Qyl.Api.Contracts.OTel.Enums;\nusing Qyl.Api.Contracts.OTel.Logs;\nusing Qyl.Api.Contracts.OTel.Metrics;\nusing Qyl.Api.Contracts.Runner;\nusing Qyl.Api.Contracts.Runner.Mcp;\n#pragma warning disable MCPEXP001\n` +
        ...
        `var catalogContractValid = catalogUnionTypesValid && catalogSourceTypesValid && catalogEconomicsTypesValid && catalogEnumWireValid && candidateStatusValid && catalogWireValid && candidateWireValid && otlpFidelityValid;\n` +
