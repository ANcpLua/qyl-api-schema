  ...
  try {
    ...
    await writeFile(
      ...
      `import { GenAiEtlCandidateStatusValues, GenAiEtlEvidenceSignalValues, GenAiEtlFrontierCostBasisValues, GenAiEtlValidationMetricValues, HealthStatusValues, ModelCatalogBillingModeValues, ModelCatalogMatchKindValues, ModelCatalogObservedIdentityBasisValues, ModelCatalogPriceSemanticsValues, ModelCatalogRateRelationValues, ModelCatalogTokenExclusionReasonValues, ProblemDetailsMediaType, ProviderBillingAttributionValues, ProviderBillingSourceStatusValues, RunnerMcpEvaluationExportFormatValues, RunnerMcpExecutionStatusValues, RunnerMcpTransportKindValues, RunnerResourceKindValues, RunnerResourceLifecycleValues } from ${JSON.stringify(`${npmPackage}/types`)};\n` +
        ...
        `const exactMetricContract = JSON.stringify(metricPointRefs) === JSON.stringify(expectedMetricPointRefs) && JSON.stringify(metricValueRefs) === JSON.stringify(["#/$defs/OTel.Metrics.MetricIntegerValue", "#/$defs/OTel.Metrics.MetricDoubleValue"]) && schema.$defs["OTel.Metrics.MetricInt64"]?.type === "string" && schema.$defs["OTel.Metrics.MetricUInt64"]?.type === "string" && schema.$defs["OTel.Metrics.MetricUnixNanos"]?.type === "string" && schema.$defs["OTel.Metrics.MetricStartUnixNanos"]?.type === "string" && schema.$defs["OTel.Metrics.HistogramMetricPoint"]?.properties?.bucket_counts?.items?.$ref === "#/$defs/OTel.Metrics.MetricUInt64";\n` +
        ...
        `const auditRequestBody = schema.$defs["Operations.GenAiEtlAuditApi_evaluate.Request"];\n` +
        `const metricsResponseBody = schema.$defs["Operations.MetricsApi_list.Response.200"];\n` +
        `const operationBodyContracts = auditRequestBody?.$ref === "#/$defs/Cost.GenAiEtlAuditEvaluationRequest" && metricsResponseBody?.required?.includes("items") && metricsResponseBody.required.includes("has_more") && metricsResponseBody.properties?.items?.items?.$ref === "#/$defs/OTel.Metrics.MetricPoint" && JSON.stringify(metricsResponseBody.unevaluatedProperties) === JSON.stringify({ not: {} });\n` +
        ...
        `if (ProblemDetailsMediaType !== "application/problem+json" || !errorsOwnProblemJson || !workbenchCookieAuth || !typedBootstrapCookie || !logStreamCapacityResponseDeclared || !typedQueryValidationResponsesDeclared || !metricsListDeclared || !exactMetricContract || !eventLogContract || !operationBodyContracts || !exactServerConfigurationUnion || !exactAssertionUnion || !exactExportUnion || !opaqueSdkBoundaries || !recursiveAttributeValue || !compoundValidationMetrics || !qualifiedCandidate || !aggregateBilling || !requiredCatalogSources || !separatedCatalogEstimate || !catalogSummaryMetrics || !separatedFrontierBasis || GenAiEtlValidationMetricValues.calibrationError !== "calibration_error" || GenAiEtlValidationMetricValues.spanPrecision !== "span_precision" || GenAiEtlValidationMetricValues.spanRecall !== "span_recall" || HealthStatusValues.healthy !== "healthy" || RunnerResourceLifecycleValues.ready !== "ready" || RunnerResourceKindValues.stdio !== "stdio" || RunnerMcpTransportKindValues.streamableHttp !== "streamable_http" || RunnerMcpExecutionStatusValues.timedOut !== "timed_out" || RunnerMcpEvaluationExportFormatValues.report !== "report" || !schema.$defs["Mcp.Tools.FetchTelemetryInput"]) process.exit(1);\n`,
    );
    run("node", ["smoke.mjs"], npmDir);
    await writeFile(
      join(npmDir, "smoke.ts"),
      `import type { HistogramMetricPoint, LogRecord, MetricIntegerValue, MetricPoint } from ${JSON.stringify(`${npmPackage}/types`)};\n` +
        ...
        `const histogram: HistogramMetricPoint = { type: "histogram", name: "gen_ai.client.token.usage", unit: "{token}", start_time_unix_nano: "1", time_unix_nano: "2", flags: 0, resource: { "service.name": "orders" }, count: "18446744073709551615", bucket_counts: ["0", "18446744073709551615"], explicit_bounds: [128], aggregation_temporality: 1 };\n` +
        ...
        `void [eventLog, integerValue, point, invalidIntegerValue, invalidHistogram, invalidTimestamp];\n`,
    );
    run(
      process.execPath,
      [
    ...
    await writeFile(
      ...
      `using System.Text.Json;\nusing System.Text.Json.Nodes;\nusing System.Text.Json.Serialization;\nusing ModelContextProtocol;\nusing ModelContextProtocol.Protocol;\nusing Qyl.Api.Contracts.Common.Errors;\nusing Qyl.Api.Contracts.Common.Pagination;\nusing Qyl.Api.Contracts.Cost;\nusing Qyl.Api.Contracts.Health;\nusing Qyl.Api.Contracts.Mcp.Tools;\nusing Qyl.Api.Contracts.OTel.Enums;\nusing Qyl.Api.Contracts.OTel.Logs;\nusing Qyl.Api.Contracts.OTel.Metrics;\nusing Qyl.Api.Contracts.Runner;\nusing Qyl.Api.Contracts.Runner.Mcp;\n#pragma warning disable MCPEXP001\n` +
        ...
        `var eventLog = new LogRecord { TimeUnixNano = 2, ObservedTimeUnixNano = 3, SeverityNumber = SeverityNumber.Info, Body = new LogBodyString { StringValue = "evaluation completed" }, EventName = "gen_ai.evaluation.result", Resource = new Qyl.Api.Contracts.OTel.Resource.Resource { ServiceName = "evaluator" } };\n` +
        `var eventLogWire = JsonSerializer.Serialize(eventLog);\n` +
        `MetricPoint metricPoint = new HistogramMetricPoint { Name = "gen_ai.client.token.usage", Unit = "{token}", StartTimeUnixNano = 1, TimeUnixNano = 2, Flags = 0, Resource = new Qyl.Api.Contracts.OTel.Resource.Resource { ServiceName = "orders" }, Count = ulong.MaxValue, BucketCounts = [0, ulong.MaxValue], ExplicitBounds = [128], AggregationTemporality = AggregationTemporality.Delta };\n` +
        `var metricPage = new CursorPageMetricPoint { Items = [metricPoint], HasMore = false };\n` +
        ...
        `if (!catalogContractValid || ProblemDetailsMediaType.Value != "application/problem+json" || !eventLogWire.Contains("\\\"event_name\\\":\\\"gen_ai.evaluation.result\\\"") || metricRoundTrip?.Items[0] is not HistogramMetricPoint { Count: ulong.MaxValue } || !metricWire.Contains("\\\"time_unix_nano\\\":\\\"2\\\"") || !metricWire.Contains("\\\"count\\\":\\\"18446744073709551615\\\"") || !metricWire.Contains("\\\"bucket_counts\\\":[\\\"0\\\",\\\"18446744073709551615\\\"]") || !ambiguousMetricValueRejected || typeof(FetchTelemetryInput).Namespace != "Qyl.Api.Contracts.Mcp.Tools" || typeof(GenAiEtlAuditCluster).GetProperty("ValidationMetric") is not null || validationMetricsProperty?.PropertyType != typeof(IReadOnlyList<GenAiEtlValidationMetric>) || validationMetricsWireName != "validation_metrics" || validationMetricsWire != "[\\\"calibration_error\\\",\\\"span_precision\\\",\\\"span_recall\\\"]" || health != "\\\"healthy\\\"" || lifecycle != "\\\"ready\\\"" || kind != "\\\"stdio\\\"" || state.Kind != RunnerResourceKind.Stdio || toolInput.View != FetchTelemetryView.Traces || serverConfigurationRoundTrip is not RunnerMcpStdioServerConfiguration || assertionRoundTrip is not RunnerMcpStatusAssertion || exportPayloadRoundTrip is not RunnerMcpEvaluationReportExportPayload || !serverConfigurationWire.Contains("\\\"transport\\\":\\\"stdio\\\"") || !serverConfigurationWire.Contains("\\\"environmentVariable\\\":\\\"QYL_MCP_API_TOKEN\\\"") || !assertionWire.Contains("\\\"kind\\\":\\\"status\\\"") || !exportPayloadWire.Contains("\\\"format\\\":\\\"report\\\"") || executionRequest.ToolName != "inspect" || executionRequest.TimeoutMs != 30000 || !executionRequestWire.Contains("\\\"trace_id\\\":\\\"abc\\\"")) return 1;\nreturn 0;\n`,
    );
    run(
      "dotnet",
